...
def windows_switch_desktops(direction: str) -> None:
     ...
     Args:
          direction (str): The direction to switch desktops. Can be 'right' or 
                                 'left'.
     """
     keyboard.press(Key.cmd)
...
def get_trigger_area(monitor: screeninfo.common.Monitor,
                            vertical_margin: int = 1) -> list:
     """
     Get the trigger area for switching desktops.

     Args:
          monitor (screeninfo.common.Monitor): The monitor object.
          vertical_margin (int): The vertical margin for the trigger area. Default 
                                         is 1 pixel.

     Returns:
          list: The coordinates of the trigger area 
...
def is_point_inside_rectangle(point: list, rectangle: list) -> bool:
     ...
     if (x_left_lower <= x <= x_right_upper 
...
def on_scroll(x: int, y: int, dx: int, dy: int,
     ...
     """
     Handles mouse scroll events for switching desktops based on the scroll 
     direction and position.

     This function checks whether the scroll event occurs within any defined 
     monitor regions. If so, it determines the desired desktop switch direction 
     based on the vertical scroll delta (dy). A desktop switch is triggered if 
     sufficient time has elapsed since the last scroll (as per scroll_delay) or if a 
     repeated scroll in the same direction occurs after the repeat_delay.
     ...
     Args:
          ...
          scroll_delay (float, optional): The minimum delay between processing 
                                                     consecutive scroll events. Default is 0.1.
          repeat_delay (float, optional): The additional delay required before 
                                                     processing a repeated scroll event in the 
                                                     same direction. Default is 0.2.

     Returns:
          None

     Side Effects:
          Updates global variables 'last_scroll_time', 'last_move', and 
          'last_trigger_time'. Calls 'switch_desktops(move)' to switch desktops based 
     ...
     if delta_trigger > scroll_delay:
          ...
          if any(is_point_inside_rectangle((x, y), monitor)
                    for monitor in monitors):
                move = 'right' if dy < 0 else 'left'
                # Don't wait if direction has changed; wait if repeated.
                if (last_move != move or 
                          time.time() - last_scroll_time >= repeat_delay):
...
def on_move(x: int, y: int, delay: float = 0.5) -> None:
     ...
     Args:
          ...
          delay (float): Delay for triggering the hot corner action. Default is 0.5.
     """
...
if __name__ == '__main__':
     ...
     parser.add_argument(
          ...
          type=float,
          default=0.1,
          help=("Minimum delay between processing consecutive scroll events "
                  "(default: 0.1 seconds)."),
     )
     parser.add_argument(
          "--repeat_delay",
          type=float,
          default=0.3,
          help=("Additional delay for repeated scroll events in the same direction "
                  "(default: 0.2 seconds)."),
     )
     parser.add_argument(
          "--hot_corner_delay",
          type=float,
          default=0.5,
          help=("Delay for triggering the hot corner action "
                  "(default: 0.5 seconds)."),
     )
     args = parser.parse_args()

     # Override the on_scroll function to include scroll_delay and repeat_delay args
